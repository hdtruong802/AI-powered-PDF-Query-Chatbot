...
if "models_loader" not in st.session_state:
    st.session_state.models_loader = None
...
def process_pdf(uploaded_file):
    ...
    # Tạo tên thư mục lưu vector DB từ tên file
    pdf_name = uploaded_file.name
    pdf_base_name = os.path.splitext(os.path.basename(pdf_name))[0]
    persist_path = os.path.join("chroma_dbs", pdf_base_name)

    # Nếu vector DB đã tồn tại → load lại
    if os.path.exists(persist_path):
        ...
        os.unlink(temp_file_path)  # Xoá file PDF tạm
        docs_count = len(vector_db.get()['ids'])  # Đếm số chunks
    else:
        # Load tài liệu từ PDF
        loader = PyPDFLoader(temp_file_path)
        documents = loader.load()

        # Chia nhỏ bằng SemanticChunker
        ...
        # Tạo vector DB và lưu vào ổ cứng
